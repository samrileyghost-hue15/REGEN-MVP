...
export interface MaintenanceVerification {
  ...
  notes?: string;
  post_strain_reading?: number;
  tc_status_confirmed?: string;
  created_at: string;
}
...
export interface NetworkMapNode {
  id: string;
  type: 'line' | 'section' | 'tc' | 'signal' | 'asset';
  label: string;
  status: string;
  x: number;
  y: number;
  children?: NetworkMapNode[];
}
