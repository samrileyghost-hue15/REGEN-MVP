...
import type { Role } from '../types'; const ROLES: { value: Role; label: string; description: string }[] = [
  {
    value: 'maintenance_engineer',
    label: 'Maintenance Engineer',
    description: 'Detect → Inspect → Repair',
  },
  {
    value: 'operations_manager',
    label: 'Operations Manager',
    description: 'Monitor → Prioritise → Coordinate',
  },
  {
    value: 'administrator',
    label: 'Administrator',
    description: 'Monitor System → Manage Users → Configure',
  },
];
      ...
      <div className="relative z-10 w-full max-w-md px-6">
        ...
        <div className="text-center mb-10">
          ...
          <div
            ...
            style={{ background: 'linear-gradient(90deg, transparent, #00FFC6, transparent)' }}
          />
        </div>
        ...
        >
          <p className="text-xs text-text-secondary mb-5 tracking-widest uppercase">
            Select your role
          </p>
              ...
              <button
                ...
                className={`
                  ...
                  ${selectedRole === role.value
                    ...
                    : 'border-border bg-surface2 hover:border-cyan/30'
                  }
                `}
                style={selectedRole === role.value ? { background: 'rgba(0,255,198,0.06)' } : {}}
                aria-pressed={selectedRole === role.value}
              >
                <div className="flex items-center justify-between">
                  <div>
                    <p className={`text-sm font-semibold ${selectedRole === role.value ? 'text-cyan' : 'text-text-primary'}`}>
                      {role.label}
                    </p>
                    <p className="text-xs text-text-dim mt-0.5">{role.description}</p>
                  </div>
                  <div
                    className={`w-4 h-4 rounded-full border-2 transition-all ${selectedRole === role.value
                      ? 'border-cyan bg-cyan'
                      : 'border-border'
                      }`}
                  />
          ...
          <button
            onClick={handleEnter}
            className="btn-primary w-full justify-center py-3 text-sm tracking-widest"
          >
            ENTER REGEN
          </button>
