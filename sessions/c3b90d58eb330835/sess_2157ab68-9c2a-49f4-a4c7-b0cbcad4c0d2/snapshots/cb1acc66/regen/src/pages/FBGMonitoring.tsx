  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`card border-l-4 ${
        sensor.risk_level === 'SIMULATED_CRITICAL' ? 'border-l-critical'
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4">
            ...
            <div className="col-span-1">
              ...
              <p className={`mt-1 text-2xl font-bold ${
                sensor.current_strain == null ? 'text-offline'
            ...
            {[1, 7, 14, 30].map(d => (
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded border font-medium transition-colors ${
                  days === d ? 'bg-[#145DA0] text-white border-[#145DA0]'
        ...
        {filtered.map(sensor => {
          ...
          return (
            <Link
              key={sensor.id}
              to={`/fbg/${sensor.id}`}
              className={`card hover:shadow-md transition-shadow border-l-4 block ${
                sensor.risk_level === 'SIMULATED_CRITICAL' ? 'border-l-critical'
                : sensor.risk_level === 'SIMULATED_WARNING' ? 'border-l-warning'
                : sensor.sensor_status === 'OFFLINE' ? 'border-l-offline'
              ...
              <div className="p-4">
                ...
                <div className="mt-3 grid grid-cols-3 gap-2 text-center">
                  ...
                  <div>
                    ...
                    <p className={`text-sm font-bold ${
                      (pctChange ?? 0) > 30 ? 'text-critical' : 'text-healthy'
                    }`}>
                      {pctChange != null ? `${pctChange > 0 ? '+' : ''}${pctChange.toFixed(1)}%` : '—'}
                    </p>
                  </div>
